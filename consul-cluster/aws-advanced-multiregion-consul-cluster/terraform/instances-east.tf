data "template_file" "consul_update_east" {
...
resource "aws_instance" "consul_client" {
  ...
  user_data              = "${data.template_file.consul_update_east.rendered}"
...
resource "aws_instance" "consul_0" {
  provider               = "aws.east"
  instance_type          = "${var.instance_type}"
  ami                    = "${data.atlas_artifact.consul_multiregion.metadata_full.region-us-east-1}"
  key_name               = "${aws_key_pair.east_main.key_name}"
