data "template_file" "west_consul_update" {
...
resource "aws_instance" "west_consul_client" {
  ...
  ami                    = "${data.atlas_artifact.consul_client.metadata_full.region-us-west-2}"
  key_name               = "${aws_key_pair.west_main.key_name}"

  user_data              = "${data.template_file.west_consul_update.rendered}"
...
resource "aws_instance" "west_consul_0" {
  provider               = "aws.west"
  instance_type          = "${var.instance_type}"
  ami                    = "${data.atlas_artifact.consul_multiregion.metadata_full.region-us-west-2}"
  key_name               = "${aws_key_pair.west_main.key_name}"
