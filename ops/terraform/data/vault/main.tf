...
resource "aws_instance" "vault" {
  ...
  user_data     = "${element(template_file.vault_user_data.*.rendered, count.index)}"
  ...
  /*
  # Example using user_data to upload certs rather than remote-exec, not currently being used
  user_data = <<END
#!/bin/bash

echo '${file("${var.ssl_cert_crt}")}' | sudo tee /usr/local/share/ca-certificates/${var.ssl_cert_name}-test.crt > /dev/null
sudo update-ca-certificates
echo '${file("${var.ssl_cert_crt}")}' | sudo tee /usr/local/etc/${var.ssl_cert_name}-test.crt > /dev/null
echo '${file("${var.ssl_cert_key}")}' | sudo tee /usr/local/etc/${var.ssl_cert_name}-test.key > /dev/null
END

  # We're currently setting the cert in Packer, uncomment this if you want to do it in Terraform
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "echo '${file("${var.ssl_cert_crt}")}' | sudo tee /usr/local/share/ca-certificates/${var.ssl_cert_name}.crt > /dev/null",
      "sudo update-ca-certificates",
      "echo '${file("${var.ssl_cert_crt}")}' | sudo tee /usr/local/etc/${var.ssl_cert_name}.crt > /dev/null",
      "echo '${file("${var.ssl_cert_key}")}' | sudo tee /usr/local/etc/${var.ssl_cert_name}.key > /dev/null",

      # wait for vault respawns to catch before continuing
      "sleep 5",
    ]
  }
  */
